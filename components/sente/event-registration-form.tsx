...
export function EventRegistrationForm({
    ...
    const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // FormData natif chope automatiquement le hidden input "phone" du PhoneInput.
        // On override les autres champs gérés en state local.
        const fd = new FormData(e.currentTarget);
        fd.set("event_id", eventId);
        fd.set("full_name", fullName.trim());
        fd.set("payment_method", isFree ? "free" : paymentMethod);

        // Nettoyer phone si vide
        const phoneValue = String(fd.get("phone") ?? "").trim();
        if (!phoneValue) fd.delete("phone");

        // Nettoyer notes si vide
        if (notes.trim()) {
    ...
    return (
        <form onSubmit={onSubmit} className="space-y-6">
            ...
            <PhoneInput
                ...
                hint="Pour que l'organisateur puisse te contacter en cas de besoin."
            />
