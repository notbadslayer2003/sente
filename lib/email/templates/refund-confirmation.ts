...
export function buildRefundConfirmationEmail(data: RefundConfirmationData) {
    const text = `Bonjour ${data.pecheurName},

${data.etangName} t'a remboursé ${data.refundAmountEur.toFixed(2)} €.
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border:1px solid #e5e4df;">
        ...
        <tr><td style="padding:0 40px;">
          ...
          <p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#444;">
            <strong>${data.etangName}</strong> a procédé au remboursement de ton abonnement.
          </p>
          <table style="margin:24px 0;width:100%;border-collapse:collapse;border:1px solid #e5e4df;">
            <tr><td style="padding:14px 20px;background:#f5f4f0;border-bottom:1px solid #e5e4df;font-size:11px;color:#666;text-transform:uppercase;letter-spacing:0.15em;">Montant remboursé</td></tr>
        ...
        <tr><td style="padding:0 40px 40px 40px;border-top:1px solid #e5e4df;padding-top:24px;">
          <p style="margin:0;font-size:13px;color:#888;line-height:1.5;">
            Pour toute question, contacte directement ${data.etangName}.
          </p>
        </td></tr>
      </table>
